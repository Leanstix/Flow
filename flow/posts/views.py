...
from .models import Comment, Hashtag, Like, Post, Report
from .serializers import CommentSerializer, HashtagSerializer, PostSerializer
from .services import sync_comment_entities, sync_post_entities
...
def post_queryset():
    return Post.objects.select_related(
        'user', 'reposted_from', 'reposted_from__user'
    ).prefetch_related(
        'likes', 'comments', 'reposts', 'media', 'hashtags', 'mentioned_users',
        'reposted_from__media', 'reposted_from__hashtags',
    )


def comment_queryset():
    return Comment.objects.select_related(
        'user', 'post', 'parent', 'parent__user', 'root'
    ).prefetch_related('mentioned_users')


def serialize_post(post, request, status_code=status.HTTP_200_OK):
    hydrated = post_queryset().get(pk=post.pk)
    return Response(PostSerializer(hydrated, context={'request': request}).data, status=status_code)
...
def add_comment(request, post_id):
    ...
    sync_comment_entities(comment, request.user)
    ...
    hydrated = comment_queryset().get(pk=comment.pk)
    return Response(CommentSerializer(hydrated).data, status=status.HTTP_201_CREATED)
...
def reply_to_comment(request, comment_id):
    parent_comment = get_object_or_404(
        comment_queryset().select_related('post__user'),
        id=comment_id,
    )
    serializer = CommentSerializer(data=request.data)
    serializer.is_valid(raise_exception=True)
    reply = serializer.save(post=parent_comment.post, parent=parent_comment, user=request.user)
    mentioned = sync_comment_entities(reply, request.user)
    ...
    for mentioned_user in mentioned:
        notified_users.pop(mentioned_user.id, None)
    ...
    hydrated = comment_queryset().get(pk=reply.pk)
    return Response(CommentSerializer(hydrated).data, status=status.HTTP_201_CREATED)
...
def repost(request, post_id):
    ...
    sync_post_entities(new_post, request.user)
...
class SearchPostsView(APIView):
    ...
    def get(self, request):
        ...
        if query.startswith('#'):
            posts = post_queryset().filter(hashtags__name__iexact=query[1:])
        else:
            posts = post_queryset().filter(
                Q(content__icontains=query)
                | Q(user__user_name__icontains=query)
                | Q(user__email__icontains=query)
                | Q(hashtags__name__icontains=query.lstrip('#'))
            )
        posts = posts.exclude(user=request.user).distinct().order_by('-created_at')
...
class HashtagSearchView(APIView):
    ...
    def get(self, request):
        query = request.query_params.get('q', '').strip().lower().lstrip('#')
        hashtags = Hashtag.objects.annotate(posts_count=Count('posts', distinct=True))
        if query:
            hashtags = hashtags.filter(name__icontains=query)
        hashtags = hashtags.order_by('-posts_count', 'name')[:30]
...
class HashtagPostsView(APIView):
    ...
    def get(self, request, tag):
        posts = post_queryset().filter(hashtags__name__iexact=tag.lstrip('#')).distinct().order_by('-created_at')
        paginator = CustomPagination()
...
class SearchUserPostsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if query:
            user_posts = user_posts.filter(Q(content__icontains=query) | Q(hashtags__name__icontains=query.lstrip('#'))).distinct()
        user_posts = user_posts.order_by('-created_at')
        paginator = CustomPagination()
        page = paginator.paginate_queryset(user_posts, request, view=self)
        serializer = PostSerializer(page, many=True, context={'request': request})
        return paginator.get_paginated_response(serializer.data)
...
def get_comments(request, post_id):
    post = get_object_or_404(Post, id=post_id)
    comments = comment_queryset().filter(post=post, parent__isnull=True).order_by('-created_at')
...
def get_comment_replies(request, comment_id):
    ...
    replies = comment_queryset().filter(root_id=root_id).exclude(id=root_id).order_by('created_at')
    paginator = CommentPagination()
    page = paginator.paginate_queryset(replies, request)
    response = paginator.get_paginated_response(CommentSerializer(page, many=True).data)
    response.data['root_id'] = root_id
    return response
