...
User = get_user_model()
ATTACHMENT_FIELDS = ('attachment_type', 'attachment_payload', 'attachment_duration_seconds', 'files')
...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({'error': 'participants must contain only valid user ids.'}, status=status.HTTP_400_BAD_REQUEST)

        participants_set.add(request.user.id)
        if len(participants_set) < 2:
            return Response({'error': 'A conversation must include at least two distinct participants.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if len(participants_set) == 2:
            ...
            ).filter(participant_count=2, participants__id__in=participants_set).distinct()
    ...
    def messages(self, request, pk=None):
        ...
        recent_messages = list(
            ...
            .prefetch_related('receipts', 'attachments', 'reply_to__attachments')
            .order_by('-timestamp')[:50]
        )
        serializer = MessageSerializer(reversed(recent_messages), many=True, context={'request': request})
    ...
    def send_message(self, request, pk=None):
        ...
        serializer = MessageSerializer(
            data={**request.data, 'conversation': conversation.id},
            context={'request': request},
        )
        serializer.is_valid(raise_exception=True)
        attachment_data = {field: serializer.validated_data.get(field) for field in ATTACHMENT_FIELDS}
        ...
        return Response(
            MessageSerializer(hydrated_message(message), context={'request': request}).data,
...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        with transaction.atomic():
            ...
            create_message_attachments(message, self.request, attachment_data)
            ensure_message_receipts(message)
            create_message_notifications(message)
            transaction.on_commit(lambda: broadcast_message(message))
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(MessageSerializer(hydrated_message(message), context={'request': request}).data)
